    ...
    fetch("/api/service/new", {
      ...
      .then(response => {
        if (!response.ok) {
          if (response.status === 401) {
            ...
            setSuccessMessage(null); // Supprime le message de succès
            return Promise.reject(); // Arrête l'exécution ici
          }
          throw new Error("Erreur lors de la soumission de la requête.");
        }
      ...
      .then(data => {
        if (data.status === "success") {
          ...
          resetForm();
          setError(null); // Réinitialise les erreurs si succès
        } else {
      ...
      .catch(error => {
        if (!error.message.includes("401")) {
          setError("Erreur lors de l'ajout du service."); // Définit un message générique si ce n'est pas une erreur 401
        }
        setSuccessMessage(null); // Supprime tout message de succès
      });
  };
  return (
