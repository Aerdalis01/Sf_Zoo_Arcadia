    ...
    fetch(`/api/service/update/${selectedServiceId}`, {
      ...
      .then((data) => {
        if (data) {
          ...
          setError(null); // Supprime le message d'erreur si succès
        }
      })
      .catch(() => {
        setError("Erreur lors de la mise à jour du sous-service."); // Définit un message d'erreur générique
        setSuccessMessage(null); // Supprime tout message de succès
        formRef.current?.reset(); // Réinitialise le formulaire si nécessaire
